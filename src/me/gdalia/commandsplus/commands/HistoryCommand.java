...
@me.gdalia.commandsplus.utils.CommandAutoRegistration.Command(value = "history")
		...
		Punishments.getInstance().getHistory(target.getUniqueId()).forEach(punishment -> {
			...
			cs.getValues(false).entrySet().forEach(entry -> {
				if (entry.getValue() instanceof String stringValue) {
					...
					sender.sendMessage(Utils.color("&e" + entry.getKey() + "&7: &b" + BooleanUtils.toStringYesNo(boolValue)));
					return;
				}
			});
		});
